...
fun StudioLabScreen(
    ...
    ToolScaffold(
        ...
        Column(
                            ...
                            scope.launch {
                                runCatching {
                                    withContext(Dispatchers.IO) {
                                        repository.applyDerivedAsset(projectId, sourceId, derived.id)
                                    }
                                }.onSuccess { updated ->
                                    project = updated
            ...
            OutlinedButton(onClick = ::reload, enabled = !processing, modifier = Modifier.fillMaxWidth()) {
                Text("Đọc lại project")
            }
        }
    }
}
            ...
            Row(
                ...
                assets.forEach { asset ->
                    FilterChip(
                        ...
                        enabled = enabled,
                        label = { Text(asset.displayName.take(28)) },
                    )
                }
            }
...
@Composable
private fun MediaToolIntegrationCard(
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(9.dp)) {
            ...
            Text(
                "Voice Cleanup, Vocal Polish và Spatial/8D tạo asset mới trong project. Source không bị ghi đè.",
                style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
            )
            Button(
            ...
            if (!voiceCleanupReady) {
                Text(
                    "Voice Cleanup chưa có model MossFormer2. Tải model một lần trong công cụ Làm sạch giọng rồi quay lại Lab.",
                    style = MaterialTheme.typography.bodySmall,
                    color = MaterialTheme.colorScheme.onSurfaceVariant,
                )
            }
...
@Composable
private fun StudioProCard(
    project: StudioProject,
    enabled: Boolean,
    onSave: (StudioProSettings) -> Unit,
    onRenderPro: (StudioProSettings) -> Unit,
) {
    ...
    var metronomeEnabled by remember(project.id, project.proSettings.tempo.metronomeEnabled) {
        mutableStateOf(project.proSettings.tempo.metronomeEnabled)
    }
    var metronomeGain by remember(project.id, project.proSettings.tempo.metronomeGainDb) {
    ...
    LaunchedEffect(previewClick, bpm, beatsPerBar) {
        ...
        while (isActive && previewClick) {
            val accent = beat % beatsPerBar.coerceAtLeast(1) == 0
    ...
    fun settings(): StudioProSettings = StudioProSettings(
        tempo = StudioTempoSettings(
            bpm = bpm.coerceIn(40f, 260f),
            beatsPerBar = beatsPerBar.coerceIn(2, 12),
            metronomeEnabled = metronomeEnabled,
            metronomeGainDb = metronomeGain.coerceIn(-36f, 0f),
        ),
        vocalFx = fx,
    )
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
            ...
            Text(
                "Tempo + metronome metadata và Vocal FX chain được lưu trong project. Render Pro tạo DERIVED asset để A/B và quay lại source bất cứ lúc nào.",
                style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
            )
            ...
            OutlinedButton(
                ...
                enabled = enabled,
                modifier = Modifier.fillMaxWidth(),
            ) { Text(if (previewClick) "Dừng click preview" else "Nghe click BPM") }
            ...
            Row(
                ...
                FilterChip(selected = false, onClick = { fx = StudioProFilterBuilder.naturalPreset() }, enabled = enabled, label = { Text("Natural") })
                FilterChip(selected = false, onClick = { fx = StudioProFilterBuilder.rapPreset() }, enabled = enabled, label = { Text("Rap") })
                FilterChip(selected = false, onClick = { fx = StudioProFilterBuilder.brightPreset() }, enabled = enabled, label = { Text("Bright") })
            ...
            FxSlider("Compressor ratio", fx.compressorRatio, String.format(Locale.US, "%.1f:1", fx.compressorRatio), 1f..8f, enabled) {
                fx = fx.copy(compressorRatio = it)
            }
            FxSlider("Reverb", fx.reverbWet, "${(fx.reverbWet * 100f).toInt()}%", 0f..0.35f, enabled) {
                fx = fx.copy(reverbWet = it)
            }

            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                OutlinedButton(onClick = { onSave(settings()) }, enabled = enabled, modifier = Modifier.weight(1f)) {
                    Text("Lưu Pro")
                }
                Button(onClick = { onRenderPro(settings()) }, enabled = enabled, modifier = Modifier.weight(1f)) {
                    Text("Render Pro → Derived")
                }
            }
        }
    }
...
private fun DerivedResultCard(
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            Text(
                "${asset.processorLabel ?: asset.processorId ?: "Processor"} • source ${asset.sourceAssetId?.take(8) ?: "?"}",
                style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
            )
...
private fun StudioProject.defaultLabSourceId(): String? {
    tracks.asSequence()
        .filter { it.type != com.aistudio.mediatool.feature.studio.domain.StudioTrackType.BEAT }
        .forEach { track ->
